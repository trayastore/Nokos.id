...
import { Server, Clock, Loader2, CheckCircle2, XCircle, ExternalLink } from "lucide-react";
...
export default function AdminPanelOrders() {
  ...
  useEffect(() => {
    ...
    return subscribeManualOrders(all => setOrders(all.filter(o => o.type === "panel")));
  }, [isAdmin, setLocation]);

  const waiting = orders.filter(o => o.status === "waiting").length;

  return (
    <AppLayout title="Pesanan Panel">
      <div className="space-y-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h2 className="text-xl font-extrabold text-foreground">Pesanan Panel</h2>
          <p className="text-sm text-muted-foreground mt-1">
            {orders.length} pesanan panel · {waiting > 0 && <span className="font-bold text-amber-600">{waiting} menunggu</span>}
          </p>
        </motion.div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-amber-700">Kelola di Pesanan Manual</p>
            <p className="text-xs text-amber-600 mt-0.5">Untuk memproses, mengirim data, dan menolak pesanan panel, gunakan halaman Pesanan Manual yang memiliki fitur lengkap.</p>
            <button onClick={() => setLocation("/admin/manual-orders")}
              className="mt-2.5 inline-flex items-center gap-1.5 text-xs font-bold text-white bg-amber-500 px-3 py-1.5 rounded-xl">
        ...
        {orders.length === 0 ? (
          <div className="text-center py-16 bg-card rounded-2xl border border-card-border">
            ...
            <p className="text-muted-foreground text-sm">Belum ada pesanan panel</p>
          </div>
        ) : (
          <motion.div variants={{ show: { transition: { staggerChildren: 0.04 } } }} initial="hidden" animate="show" className="space-y-2.5">
            {orders.map(order => {
              ...
              const wa = order.buyerWhatsapp
                ? `https://wa.me/${order.buyerWhatsapp.replace(/\D/g, "").replace(/^0/, "62")}?text=Halo+kak+${encodeURIComponent(order.userName)}%2C+pesanan+panel+kamu+ID+${order.id}`
                : null;
                ...
                <motion.div key={order.id}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm text-foreground truncate">{order.panelName || "-"}</p>
                      {order.panelVariantName && (
                        <p className="text-[11px] text-muted-foreground">{order.panelVariantName}</p>
                      )}
